...
<p>
    ...
    <div class="form-c form" id="form-c">
        <div class="images" id="images">
            <img loading="lazy" class="img-f" id="img-f" src="assets/book.svg" alt="">
        </div>

       <!-- Step 1: Booking Form -->
<!-- Step 1: Booking Form -->
       <form id="bookingForm" class="fom" method="POST" action="process_payment.php">
...
$(document).ready(function() {
    const localAmountUSD = 0.1;
    const internationalAmountUSD = 1000;
    const exchangeRate = 150; // Approximate KES/USD exchange rate

    // Function to calculate amount in KES
    function calculateAmountKES(category) {
        let amountUSD = (category === 'local') ? localAmountUSD : internationalAmountUSD;
        return amountUSD * exchangeRate;
    }

    // Update amount based on category selection
    $('#category').change(function() {
        const category = $(this).val();
        const amountKES = calculateAmountKES(category);
        $('#amount').val(amountKES); // Update the hidden input field
    });
    ...
    $('#proceedToPay').click(function(e) {
        ...
        if ($('#bookingForm')[0].checkValidity()) {
            ...
            $(this).html('<span class="btn-text">Processing...</span>');

            // AJAX call to initiate_payment.php
            $.ajax({
                url: 'initiate_payment.php', // New PHP script
                type: 'POST',
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        // Redirect the user to IntaSend's payment page
                        window.location.href = response.payment_url;
                    } else {
                        alert('Error: ' + response.message);
                        $('#proceedToPay').prop('disabled', false);
                        $('#proceedToPay').html('<span class="btn-text">Proceed To Pay</span>');
                    }
                },
